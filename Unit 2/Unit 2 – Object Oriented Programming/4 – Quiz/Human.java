...
/**
 * Name: Harry Liu
 * Description: Class for Human, contains the required attributes, constructors, and methods
 * Date: Mar 1, 2022
 */
...
public class Human {
	...
	/**
	 * Name: Human
	 * Description: Default constructor for Human, set name to empty, weight and energyLevel to -1
	 */
	public Human () {
		name = "";
		weight = -1;
	...
	public void run (double km) {
		energyLevel = (int)Math.max(0, energyLevel - 3*km);	//3% energy drop per km, energy level cannot be lower than 0%
		weight = Math.max(0, weight - 0.001*km);		//0.001kg weight drop per km, weight cannot be lower than 0kg
	}
	...
	public void eat (Vegetable veg, double grams) {
		...
		energyLevel += Math.min(100, energyLevel+(int)Math.round(caloriesGained/15.0));	//keep energy within 0-100
	}
